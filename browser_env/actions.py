...
@beartype
def create_use_skill_action(skill_name: str, skill_args: dict) -> Action:
    """Create a USE_SKILL action for calling SkillWeaver skills."""
    action = create_none_action()
    action.update({
        "action_type": ActionTypes.USE_SKILL,
                        ...
                        try:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    # Final fallback: use coordinate-based click
                                    try:
                                        element_center = obseration_processor.get_element_center(element_id, page)
                                        execute_mouse_click(element_center[0], element_center[1], page)
                                    except:
                                        # If all else fails, try clicking by role without parent
                                        page.get_by_role(node.role, name=node.name).first.click(force=True)
                else:
                    try:
                        page.get_by_role(node.role, name=node.name, exact=True).click()
...
def create_id_based_actions(action_str: str):
    valid_actions = ["click", "hover", "type", "press", "scroll", "goto", "new_tab", "go_back", "go_forward", "tab_focus", "close tab", "stop", "select", "record", "branch", "prune", "note", "use_skill"]
    ...
    for raw_action_str in action_str_list:
        if raw_action_str.startswith("use_skill"):
            # Skill argument values may themselves use bracket notation.
            action = "use_skill"
        else:
            action = (
        ...
        match action:
            ...
            case "use_skill":  # use_skill skill_name arg1=val1 arg2=val2
                # Parse: use_skill navigate_to_subreddit subreddit_name="python"
                # Format: use_skill [skill_name] [arg1]=[value1] [arg2]=[value2] ...
                # Also supports list syntax: forum_names=["item1", "item2"]
                match = re.search(r"use_skill\s+(\w+)(.*)$", raw_action_str, re.DOTALL)
                if not match:
                    raise ActionParsingError(f"Invalid use_skill action {raw_action_str}")

                skill_name = match.group(1)
                args_str = match.group(2).strip()

                # Parse arguments: arg1="value1" arg2="value2" or arg1=["v1", "v2"]
                skill_args = {}
                if args_str:
                    # First, try to match list arguments: key=["value1", "value2"] or key=['value1', 'value2']
                    # This pattern matches: key=[...] where [...] contains quoted strings
                    list_pattern = r'(\w+)=(\[(?:[^\[\]]*)\])'
                    for list_match in re.finditer(list_pattern, args_str):
                        key = list_match.group(1)
                        list_str = list_match.group(2)
                        try:
                            # Try to parse as JSON array
                            import json
                            value = json.loads(list_str)
                            skill_args[key] = value
                        except json.JSONDecodeError:
                            # Fallback: parse manually for single-quoted strings
                            # Extract items from ['item1', 'item2'] format
                            items = re.findall(r'["\']([^"\']*)["\']', list_str)
                            if items:
                                skill_args[key] = items
                            else:
                                # Keep as string if parsing fails
                                skill_args[key] = list_str

                    # Remove matched list patterns from args_str to avoid double-matching
                    args_str_remaining = re.sub(list_pattern, '', args_str).strip()

                    # Then match regular string arguments: key="value" or key='value' or key=value
                    arg_pattern = r'(\w+)=(?:"([^"]*)"|\'([^\']*)\'|(\S+))'
                    for arg_match in re.finditer(arg_pattern, args_str_remaining):
                        key = arg_match.group(1)
                        # Skip if already parsed as list
                        if key in skill_args:
                            continue
                        # Get the value from whichever group matched (quoted or unquoted)
                        # Use 'is not None' to handle empty strings correctly
                ...
                action_cmds.append(create_use_skill_action(skill_name, skill_args))
            case "note":
                pass
