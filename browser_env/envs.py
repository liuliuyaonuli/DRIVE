...
class ScriptBrowserEnv(Env[dict[str, Observation], Action]):
    ...
    def setup(self, config_file: Path | None = None) -> None:
        try:
            ...
            # 替换环境变量占位符
            if start_url:
                start_url = start_url.replace("__SHOPPING__", os.environ.get("SHOPPING", "__SHOPPING__"))
                start_url = start_url.replace("__SHOPPING_ADMIN__", os.environ.get("SHOPPING_ADMIN", "__SHOPPING_ADMIN__"))
                start_url = start_url.replace("__REDDIT__", os.environ.get("REDDIT", "__REDDIT__"))
                start_url = start_url.replace("__GITLAB__", os.environ.get("GITLAB", "__GITLAB__"))
                start_url = start_url.replace("__MAP__", os.environ.get("MAP", "__MAP__"))
                start_url = start_url.replace("__WIKIPEDIA__", os.environ.get("WIKIPEDIA", "__WIKIPEDIA__"))
                start_url = start_url.replace("__HOMEPAGE__", os.environ.get("HOMEPAGE", "__HOMEPAGE__"))
    ...
    # @beartype  # 禁用类型检查，因为 observation_handler 返回的类型可能不完全符合类型提示
    def reset(
        self,
        *,
        seed: int | None = None,
