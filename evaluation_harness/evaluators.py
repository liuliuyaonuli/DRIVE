...
try:
    ...
    print("Trying to use existing punkt data if available...")
...
class StringEvaluator(Evaluator):
    ...
    def exact_match(ref: str, pred: str) -> float:
        """Apply WebArena's exact-answer criterion after quote/case cleanup."""
        clean_ref = StringEvaluator.clean_answer(ref)
        clean_pred = StringEvaluator.clean_answer(pred)
        return float(clean_pred == clean_ref)
...
class URLEvaluator(Evaluator):
    """Check URL matching with support for functionally equivalent URLs"""
    ...
    def __call__(
        ...
        def clean_url(url: str) -> str:
            url = str(url)
            # Replace placeholders with actual URLs from environment
            url = url.replace("__GITLAB__", os.environ.get("GITLAB", "http://127.0.0.1:8023"))
            url = url.replace("__REDDIT__", os.environ.get("REDDIT", "http://127.0.0.1:9999"))
            url = url.replace("__SHOPPING__", os.environ.get("SHOPPING", "http://127.0.0.1:7770"))
            url = url.replace("__SHOPPING_ADMIN__", os.environ.get("SHOPPING_ADMIN", "http://127.0.0.1:7770/admin"))
            url = url.replace("__MAP__", os.environ.get("MAP", "http://127.0.0.1:3000"))
            url = url.replace("__WIKIPEDIA__", os.environ.get("WIKIPEDIA", "http://127.0.0.1:8888/wikipedia_en_all_maxi_2022-05/A/User:The_other_Kiwix_guy/Landing"))
            url = url.replace("__HOMEPAGE__", os.environ.get("HOMEPAGE", "http://127.0.0.1:4399"))
            url = url.rstrip("/")
            url = url.lower()
            return url
    ...
    def __call__(
        ...
        for target in targets:
            ...
            for or_target in or_target_list:
                ...
                # Replace environment variable placeholders in target_url
                import os
                import re
                env_vars = ["REDDIT", "SHOPPING", "SHOPPING_ADMIN", "GITLAB", "WIKIPEDIA", "MAP", "HOMEPAGE"]
                for env_var in env_vars:
                    placeholder = f"__{env_var}__"
                    if placeholder in target_url:
                        env_value = os.environ.get(env_var, "")
                        if env_value:
                            target_url = target_url.replace(placeholder, env_value)
                        else:
